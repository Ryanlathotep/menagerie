 ...
 * and the SAME MonsterSprite renderer used everywhere else in the game, so the
 * arena reads visually as part of the world instead of a separate mini-game.
 */
import { useMemo } from 'react';
import type { ArenaReplay } from './types';
import { FloorTile, WallTile } from '@/game/TileGraphics';
...
export function ArenaBoard({ replay, currentEventIndex, width = 24, height = 24 }: ArenaBoardProps) {
  ...
  const state = useMemo(
    () => rebuildState(replay, currentEventIndex, width, height),
    [replay, currentEventIndex, width, height],
  );
  ...
  const framedW = width + 2;
  const framedH = height + 2;

  return (
    <div
      className="relative w-full max-w-2xl mx-auto rounded-lg overflow-hidden border-2 border-amber-900/40"
          ...
          {state.combatants.map((c) => {
            ...
            return (
              ...
              >
                ...
                {/* HP bar */}
                <div
                  className="absolute left-1 right-1 rounded-sm overflow-hidden border border-black/40"
                  style={{ bottom: -2, height: 4, background: 'hsl(0 0% 15% / 0.5)' }}
                >
                  <div
