          ...
          <ScrollArea className="h-48">
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
              {unlockedMonsters.map((monster) => (
                <Card
                  key={monster.comboId}
                  className={`p-3 cursor-pointer transition-all ${selectedMonster?.comboId === monster.comboId ? "ring-2 ring-primary bg-primary/10" : "hover:border-primary/50"}`}
          ...
          <Card className="p-4">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-bold text-lg capitalize">{selectedMonster.species}</h3>
    ...
    <>
      ...
      <div className={`fixed inset-0 ${bottomOffset} overflow-hidden transition-all duration-300`}>
        <div className="h-full flex flex-col">
          ...
          <div
            className={`fixed ${controlsOffset} left-0 right-0 h-[160px] bg-card border-t-2 border-primary/20 p-4 z-40 transition-all duration-300`}
          >
            {/* Mobile controls */}
            <div className="grid grid-cols-3 gap-2 w-32 mx-auto sm:hidden mb-3">
              <div />
              <Button size="sm" onClick={() => handleMove("up")}>
                ↑
              </Button>
              <div />
              <Button size="sm" onClick={() => handleMove("left")}>
                ←
              </Button>
              <div />
              <Button size="sm" onClick={() => handleMove("right")}>
                →
              </Button>
              <div />
              <Button size="sm" onClick={() => handleMove("down")}>
                ↓
              </Button>
              <div />
            </div>

            <p className="text-muted-foreground text-sm hidden sm:block text-center mb-2">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex gap-1">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-muted">{battle.enemyMonster.class}</span>
                  </div>
                </div>
                ...
                <p className="text-sm mt-1 font-mono">
                  {battle.enemyMonster.stats.currentHp} / {battle.enemyMonster.stats.maxHp} HP
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card className="p-4 border-2 border-primary/50 flex flex-col">
            <div className="flex items-center gap-4 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm mt-1 font-mono">
                  {battle.playerMonster.stats.currentHp} / {battle.playerMonster.stats.maxHp} HP
                </p>
                {/* Stamina Bar */}
        ...
        {showInventory && (
          <div className="border-b border-primary/10 p-3">
            <div className="flex justify-between items-center mb-2">
              ...
              <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setShowInventory(false)}>
                ✕
              </Button>
            </div>
            {inventory.length === 0 ? (
              ...
              <div className="flex gap-2 flex-wrap">
                {inventory.map((item, i) => (
                  <Button key={i} variant="outline" size="sm" className="text-xs" onClick={() => handleUseItem(item)}>
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3 items-start">
                ...
                return (
                  <MoveTooltip key={move.id} move={move} attacker={battle.playerMonster} defender={battle.enemyMonster}>
                    <Button
                      ...
                      className={`h-auto py-2 px-3 text-left flex-shrink-0 ${!canAfford && move.id !== "struggle" ? "opacity-50" : ""} ${move.id === "struggle" ? "border-destructive text-destructive" : ""}`}
                    ...
                    >
                      <div>
                        ...
                        <p className="text-[9px] opacity-70">
                          ...
                          🎯{move.accuracy}% ⚡{move.staminaCost}
                          {move.type === "heal" ? " 💚" : ""}
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              <Button variant="destructive" size="sm" onClick={handleFlee} className="flex items-center gap-1">
                ...
                {Math.min(
                  95,
                  Math.max(5, 50 + (battle.playerMonster.stats.speed - battle.enemyMonster.stats.speed) * 2),
                )}
                %)
              </Button>
            </div>
