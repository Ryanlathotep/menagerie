...
export const BUILDING_DEFINITIONS: Record<PlayerBuildingType, BuildingDefinition> = {
  ...
  stone_staircase: {
    ...
    emoji: '🪜',
    description: 'Overworld climb: place ON a cliff face to open a passable step, or beside a wall / cliff to anchor a wall-top climb. Separate from dungeon floor stairs.',
    cost: { wood: 0, stone: 6 },
    maxHp: 40,
    category: 'utility',
  },
  ladder: {
    ...
    name: 'Cliff Ladder',
    emoji: '🪜',
    description: 'Overworld climb: place ON a cliff face or beside a wall / cliff. Cheaper than the stone staircase, identical function.',
    cost: { wood: 4, stone: 0 },
    maxHp: 20,
    category: 'utility',
  },
...
export function canPlaceBuilding(
  ...
  // Elevation rules: waterfalls always reject; ramps reject non-stairs.
  // Cliffs reject everything EXCEPT stair/ladder — those are exactly the
  // connectors meant to open a passable route through the cliff face.
  const isStairLike = buildingType === 'stone_staircase' || buildingType === 'ladder';
  if (tileInfo?.isWaterfall) return { canPlace: false, reason: 'Cannot build on a waterfall' };
  if (tileInfo?.isRamp)      return { canPlace: false, reason: 'Ramps already provide a passable climb' };
  if (tileInfo?.isCliff && !isStairLike) {
    return { canPlace: false, reason: 'Only cliff-stairs / ladders can be placed on a cliff face' };
  }
  ...
  // Cliff-stair / ladder placement rules:
  //   • Placed ON a cliff face → replaces the cliff, becomes a passable climb.
  //   • Placed adjacent to a wall or a natural cliff → anchors to that face.
  // This is the OVERWORLD cliff connector; dungeon floor stairs (up/down/exit)
  // are separate tile types generated by dungeon.ts and cannot be built.
  if (isStairLike) {
    ...
    const onCliff = !!tileInfo?.isCliff;
    if (!adjacentWall && !adjacentCliff && !onCliff) {
      return { canPlace: false, reason: 'Place on a cliff face, or touching a wall / cliff to anchor' };
    }
