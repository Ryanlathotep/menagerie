...
import type { CraftGrid, DiscoveredRecipe, GridSize } from './types';
...
export function recordDiscovery(rec: Omit<DiscoveredRecipe, 'local'>) {
  ...
  writeLocal(list);
  // Fire cloud sync — don't block UI.
...
async function syncDiscoveryToCloud(rec: Omit<DiscoveredRecipe, 'local'>) {
  ...
  // Try to claim first-discovery. RLS + unique-hash constraint means only the
  // first inserter wins; others silently fail.
...
export async function syncCloudRecipeBook(): Promise<DiscoveredRecipe[]> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('hash, blueprint_id, item_name, grid_json, grid_size, discovered_by_username, discovered_at, world_seed')
...
export async function lookupDiscovery(hash: string): Promise<DiscoveredRecipe | null> {
  ...
  try {
    const { data } = await supabase
      ...
      .maybeSingle();
    if (!data) return null;
    const row = data as unknown as CloudRow;
    return {
      hash: row.hash,
      blueprintId: row.blueprint_id,
      itemName: row.item_name,
